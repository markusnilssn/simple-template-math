...
namespace stm
{
	...
	public:
		...
		static Vector3<T> Lerp(const Vector3<T>& A, const Vector3<T>& B, const float& T);
	...
	template <typename T>
	Vector3<T>::Vector3(const std::initializer_list<T>& aInitList)
	{
		x = 0;
		y = 0;
		z = 0;
	...
	template <typename T>
	Vector3<T> Vector3<T>::Lerp(const  Vector3<T>& a, const  Vector3<T>& b, const float& f)
	{
		return a + f * (b - a);
	}
